...
interface Props {
  ...
  Cash : any;
  updateUser: (CashFlowStatement: any) => void;
}

export const EditCashFlowStatement: React.FC<Props> = ({
  closeEditComponent,
  Cash,
  updateUser
}) => {
  const [EditCashFlowStatement, setEditCashFlowStatement] = useState(Cash);
  ...
  const initializeData = () => {
    if (FYpreInput.current) FYpreInput.current.value = Cash.FYpre;
    if (FYcurrentdInput.current) FYcurrentdInput.current.value = Cash.FYcurrent;
    if (Q1FYCurrentInput.current) Q1FYCurrentInput.current.value = Cash.Q1FYCurrent;
    if (Q2FYCurrentInput.current) Q2FYCurrentInput.current.value = Cash.Q2FYCurrent;
    if (Q3FYCurrentInput.current) Q3FYCurrentInput.current.value = Cash.Q3FYCurrent;
    if (Q4FYCurrentInput.current) Q4FYCurrentInput.current.value = Cash.Q4FYCurrent;
  };
      ...
      <div className="popup-inner" style={{overflowY:'auto', maxHeight:'100vh'}}>
        <div className="card" style={{ padding: "10px", maxWidth: "700px" }}>
          ...
          >
            ...
            <div style={{ marginLeft: "10px" }}>
              <p style={{ fontSize: "20px" }}> Cash Flow Statement </p>
            </div>
          </div>
          <div
