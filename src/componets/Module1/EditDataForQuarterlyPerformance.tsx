...
interface Props {
  ...
  Quarterly : any;
  updateUser: (QuarterlyPerformance: any) => void;
}

export const EditQuarterlyPerformance: React.FC<Props> = ({
  closeEditComponent,
  Quarterly,
  updateUser
}) => {
  const [EditQuarterlyPerformance, setEditQuarterlyPerformance] = useState(Quarterly);
  ...
  const initializeData = () => {
    if (FYpreInput.current) FYpreInput.current.value = Quarterly.FYpre;
    if (FYcurrentdInput.current) FYcurrentdInput.current.value = Quarterly.FYcurrent;
    if (Q1FYCurrentInput.current) Q1FYCurrentInput.current.value = Quarterly.Q1FYCurrent;
    if (Q2FYCurrentInput.current) Q2FYCurrentInput.current.value = Quarterly.Q2FYCurrent;
    if (Q3FYCurrentInput.current) Q3FYCurrentInput.current.value = Quarterly.Q3FYCurrent;
    if (Q4FYCurrentInput.current) Q4FYCurrentInput.current.value = Quarterly.Q4FYCurrent;
  };
      ...
      <div className="popup-inner" style={{overflowY:'auto', maxHeight:'100vh'}}>
        <div className="card" style={{ padding: "10px", maxWidth: "700px" }}>
          ...
          >
            ...
            <div style={{ marginLeft: "10px" }}>
              <p style={{ fontSize: "20px" }}> Quarterly Performance </p>
            </div>
          </div>
          <div
