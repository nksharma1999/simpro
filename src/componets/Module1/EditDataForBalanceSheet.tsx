...
interface Props {
  ...
  Balance : any;
  updateUser: (BalanceSheet: any) => void;
}

export const EditBalanceSheet: React.FC<Props> = ({
  closeEditComponent,
  Balance,
  updateUser
}) => {
  const [EditBalanceSheet, setEditBalanceSheet] = useState(Balance);
  ...
  const initializeData = () => {
    if (FYpreInput.current) FYpreInput.current.value = Balance.FYpre;
    if (FYcurrentdInput.current) FYcurrentdInput.current.value = Balance.FYcurrent;
    if (Q1FYCurrentInput.current) Q1FYCurrentInput.current.value = Balance.Q1FYCurrent;
    if (Q2FYCurrentInput.current) Q2FYCurrentInput.current.value = Balance.Q2FYCurrent;
    if (Q3FYCurrentInput.current) Q3FYCurrentInput.current.value = Balance.Q3FYCurrent;
    if (Q4FYCurrentInput.current) Q4FYCurrentInput.current.value = Balance.Q4FYCurrent;
  };
      ...
      <div className="popup-inner" style={{overflowY:'auto', maxHeight:'100vh'}}>
        <div className="card" style={{ padding: "10px", maxWidth: "700px" }}>
          ...
          >
            ...
            <div style={{ marginLeft: "10px" }}>
              <p style={{ fontSize: "20px" }}> Balance sheet</p>
            </div>
          </div>
          <div
